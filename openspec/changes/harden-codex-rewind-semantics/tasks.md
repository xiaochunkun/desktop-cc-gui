...
- [ ] 1.1 固化回退契约：`rewind success == truncation committed`（输出：共享契约说明，前后端均引用）
- [ ] 1.2 固化边界语义：锚点后数据必须从内存与磁盘同时不可见（输出：边界定义与错误码）
...
- [ ] 4.1 reopen 链路验证：仅恢复锚点及其之前内容
- [ ] 4.2 replay 链路验证：不得回放锚点后活动
- [ ] 4.3 send 链路验证：回退后首次续写只基于截断后上下文

## 5. Verification Gate (P0)

- [ ] 5.1 回归测试：rewind -> reopen
- [ ] 5.2 回归测试：rewind -> app restart -> reopen
- [ ] 5.3 回归测试：rewind -> send
