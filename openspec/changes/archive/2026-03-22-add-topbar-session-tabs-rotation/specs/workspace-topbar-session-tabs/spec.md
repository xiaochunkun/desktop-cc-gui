...
系统 MUST 对 topbar 会话 tabs 采用 `max=4` 的确定性轮转窗口，并确保 active 会话可见。

#### Scenario: activating fifth session rotates out oldest non-active tab
- **GIVEN** topbar 已显示 4 个会话 tabs
- **WHEN** 用户激活第 5 个不同会话
- **THEN** topbar MUST 保持 4 个 tabs
...
- **THEN** topbar MUST 在同一窗口中展示它们（受 `max=4` 约束）
...
系统 MUST 在每个 tab 提供关闭入口（`X`），且关闭仅影响 topbar 可见窗口。

#### Scenario: close action removes tab from topbar window only
- **WHEN** 用户点击 tab 的 `X`
...
第一阶段系统 MUST 保持固定 4 槽轮转模型，不引入 `+N` overflow 菜单。

#### Scenario: more than four sessions still uses rotation only
- **WHEN** 激活会话数量超过 4
