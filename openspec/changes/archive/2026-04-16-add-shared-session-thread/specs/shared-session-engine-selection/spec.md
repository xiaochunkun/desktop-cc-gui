...
Define how users explicitly choose the execution engine inside a `shared session`, including the V1 `Codex / Claude` support scope, sticky selection behavior, and the single-engine-per-turn boundary.
