...
Workspace Home 的引擎选择器 MUST 将 OpenCode 作为可用选项展示，不得以“即将支持”禁用。
...
#### Scenario: home and composer availability are aligned

- **GIVEN** 系统对 OpenCode 的可用性判定已得出
- **WHEN** 渲染 Workspace Home 与 Composer 引擎选择
- **THEN** 两处 MUST 呈现一致可选状态
- **AND** MUST NOT 出现一处可选另一处不可选

### Requirement: OpenCode 新建会话必须连通既有聊天链路

用户从 Workspace Home 选择 OpenCode 后新建会话时，系统 MUST 走现有 OpenCode chat 启动流程。
