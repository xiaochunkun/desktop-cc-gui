const zhPart1 = {
  ...
  settings: {
    ...
    backgroundTerminalDesc: "跟随 official unified_exec 默认值，或仅对本桌面端拉起的 Codex 进程应用覆盖。",
    backgroundTerminalMarkerDesc: "这个设置在普通保存时不再改写全局 CODEX_HOME/config.toml。",
    backgroundTerminalApplyHint: "切换后会立即应用到当前已连接的 Codex 会话，无需额外按钮。",
    backgroundTerminalOfficialActionsDesc: "下面的按钮会直接修改官方 CODEX_HOME/config.toml，而不是桌面端本地覆盖策略。",
    ...
    backgroundTerminalOfficialConfigInheritHint: "当前桌面端策略为“跟随官方默认”，所以下面的官方配置修改会同步刷新当前 Codex runtime。",
    backgroundTerminalOfficialConfigDesktopOverrideHint: "当前桌面端策略仍由本地 override 优先；下面的官方配置修改会写入文件，但不会覆盖当前 selector 语义。",
    backgroundTerminalValueEnabled: "启用",
    backgroundTerminalValueDisabled: "停用",
    backgroundTerminalRepairTitle: "检测到旧版覆盖",
    backgroundTerminalRepairDesc: "旧版本曾向官方 CODEX_HOME/config.toml 写入 unified_exec = {{value}}。恢复官方默认值会移除这条覆盖。",
    backgroundTerminalRepairDescUnknown: "旧版本曾向官方 CODEX_HOME/config.toml 写入显式 unified_exec 条目。恢复官方默认值会移除这条覆盖。",
    backgroundTerminalRepairKeep: "保留当前覆盖",
    backgroundTerminalRepairRestore: "恢复官方默认值",
    backgroundTerminalRepairConfirm: "确认从官方 CODEX_HOME/config.toml 中移除显式 unified_exec，并恢复到平台默认值？",
    backgroundTerminalRepairSuccess: "已恢复 official unified_exec 默认值。",
    backgroundTerminalRepairAppliedSuccess: "已恢复 official unified_exec 默认值，并刷新当前已连接的 inherit Codex 会话。",
    backgroundTerminalRepairStoredSuccess: "已恢复 official unified_exec 默认值；当前桌面端仍由本地 override 策略优先。",
    backgroundTerminalApplySuccess: "已应用到当前已连接的 Codex 会话。",
    backgroundTerminalOfficialWriteTitle: "写入官方 unified_exec 配置",
    backgroundTerminalOfficialWriteEnabled: "官方配置写为启用",
    backgroundTerminalOfficialWriteDisabled: "官方配置写为停用",
    backgroundTerminalOfficialWriteEnableConfirm: "确认向官方 CODEX_HOME/config.toml 写入 unified_exec = true？这会影响其他同样读取该配置的 Codex 客户端。",
    backgroundTerminalOfficialWriteDisableConfirm: "确认向官方 CODEX_HOME/config.toml 写入 unified_exec = false？这会影响其他同样读取该配置的 Codex 客户端。",
    backgroundTerminalOfficialWriteEnabledApplied: "已把官方 unified_exec 写为启用，并刷新当前已连接的 inherit Codex 会话。",
    backgroundTerminalOfficialWriteDisabledApplied: "已把官方 unified_exec 写为停用，并刷新当前已连接的 inherit Codex 会话。",
    backgroundTerminalOfficialWriteEnabledStored: "已把官方 unified_exec 写为启用；当前桌面端仍由本地 override 策略优先。",
    backgroundTerminalOfficialWriteDisabledStored: "已把官方 unified_exec 写为停用；当前桌面端仍由本地 override 策略优先。",
